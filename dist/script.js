...
function prepareWorkspace(domain) {
  ...
  document.querySelector("#research-hint").textContent = `Certificate search is set to ${domain}. Enter this domain in ICANN Lookup. Opening these links contacts the provider.`;
}
...
form.addEventListener("submit", async event => {
  ...
  if (!allowLiveDns) {
    setStatus(`Workspace prepared for ${domain}. No lookup requests sent. Choose a provider below to load existing records.`, "success");
    return;
  }
  ...
  certificateStatus.textContent = "Live DNS was enabled for this workspace. No certificate provider contacted yet.";
